...
contract PancakeSwapRouter is IPancakeSwapRouter {
    ...
    function _addLiquidity(
        ...
        if (IPancakeSwapFactory(factoryAddr).getPair(tokenA, tokenB) == address(0)) {
            IPancakeSwapFactory(factoryAddr).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = PancakeSwapLibrary.getReserves(factoryAddr, tokenA, tokenB);
    ...
    function addLiquidityETH(
        ...
        address pair = PancakeSwapLibrary.pairFor(factoryAddr, token, WETHAddr);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IPancakeSwapPair(PancakeSwapLibrary.pairFor(factoryAddr, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        amounts = PancakeSwapLibrary.getAmountsOut(factoryAddr, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PancakeSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeSwapLibrary.pairFor(factoryAddr, path[0], path[1]), amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        amounts = PancakeSwapLibrary.getAmountsIn(factoryAddr, amountOut, path);
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETHAddr, 'PancakeSwapRouter: INVALID_PATH');
        amounts = PancakeSwapLibrary.getAmountsOut(factoryAddr, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PancakeSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETHAddr).deposit{value: amounts[0]}();
        assert(IWETH(WETHAddr).transfer(PancakeSwapLibrary.pairFor(factoryAddr, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = PancakeSwapLibrary.sortTokens(input, output);
            IPancakeSwapPair pair = IPancakeSwapPair(PancakeSwapLibrary.pairFor(factoryAddr, input, output));
            uint amountInput;
            uint amountOutput;
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETHAddr).transfer(PancakeSwapLibrary.pairFor(factoryAddr, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return PancakeSwapLibrary.getAmountsOut(factoryAddr, amountIn, path);
    }
    ...
    function getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return PancakeSwapLibrary.getAmountsIn(factoryAddr, amountOut, path);
    }
}
